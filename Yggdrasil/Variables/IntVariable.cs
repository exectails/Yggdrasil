...
namespace Yggdrasil.Variables
{
	public partial class VariableContainer<TIdent>
	{
		...
		/// different type.
		/// </exception>
		public IntVariable Int(TIdent ident)
		{
			if (!this.TryGet<IntVariable>(ident, out var variable))
			{
				if (!this.AutoCreate)
					return null;

				variable = this.Create(new IntVariable(ident));
			}
		...
		/// different type.
		/// </exception>
		public IntVariable Int(TIdent ident, int value)
		{
			if (!this.TryGet<IntVariable>(ident, out var variable))
				return this.Create(new IntVariable(ident, value));
