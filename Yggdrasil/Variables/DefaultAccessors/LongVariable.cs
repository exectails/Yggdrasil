namespace Yggdrasil.Variables.DefaultGetters
{
	...
	public static partial class VariableContainerExtensions
	{
		...
		/// different type.
		/// </exception>
		public static VariableContainer<TIdent>.LongVariable Long<TIdent>(this VariableContainer<TIdent> container, TIdent ident)
		{
			if (!container.TryGet<VariableContainer<TIdent>.LongVariable>(ident, out var variable))
			{
				if (!container.AutoCreate)
					return null;

				variable = container.Create(new VariableContainer<TIdent>.LongVariable(ident));
			}
		...
		/// different type.
		/// </exception>
		public static VariableContainer<TIdent>.LongVariable Long<TIdent>(this VariableContainer<TIdent> container, TIdent ident, long value)
		{
			if (!container.TryGet<VariableContainer<TIdent>.LongVariable>(ident, out var variable))
				return container.Create(new VariableContainer<TIdent>.LongVariable(ident, value));
