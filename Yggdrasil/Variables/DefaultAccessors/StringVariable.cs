namespace Yggdrasil.Variables.DefaultGetters
{
	...
	public static partial class VariableContainerExtensions
	{
		...
		/// different type.
		/// </exception>
		public static VariableContainer<TIdent>.StringVariable String<TIdent>(this VariableContainer<TIdent> container, TIdent ident)
		{
			if (!container.TryGet<VariableContainer<TIdent>.StringVariable>(ident, out var variable))
			{
				if (!container.AutoCreate)
					return null;

				variable = container.Create(new VariableContainer<TIdent>.StringVariable(ident));
			}
		...
		/// different type.
		/// </exception>
		public static VariableContainer<TIdent>.StringVariable String<TIdent>(this VariableContainer<TIdent> container, TIdent ident, string value)
		{
			if (!container.TryGet<VariableContainer<TIdent>.StringVariable>(ident, out var variable))
				return container.Create(new VariableContainer<TIdent>.StringVariable(ident, value));
