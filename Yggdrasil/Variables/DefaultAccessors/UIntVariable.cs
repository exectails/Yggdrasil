namespace Yggdrasil.Variables.DefaultGetters
{
	...
	public static partial class VariableContainerExtensions
	{
		...
		/// different type.
		/// </exception>
		public static VariableContainer<TIdent>.UIntVariable UInt<TIdent>(this VariableContainer<TIdent> container, TIdent ident)
		{
			if (!container.TryGet<VariableContainer<TIdent>.UIntVariable>(ident, out var variable))
			{
				if (!container.AutoCreate)
					return null;

				variable = container.Create(new VariableContainer<TIdent>.UIntVariable(ident));
			}
		...
		/// different type.
		/// </exception>
		public static VariableContainer<TIdent>.UIntVariable UInt<TIdent>(this VariableContainer<TIdent> container, TIdent ident, uint value)
		{
			if (!container.TryGet<VariableContainer<TIdent>.UIntVariable>(ident, out var variable))
				return container.Create(new VariableContainer<TIdent>.UIntVariable(ident, value));
