namespace Yggdrasil.Variables.DefaultGetters
{
	...
	public static partial class VariableContainerExtensions
	{
		...
		/// different type.
		/// </exception>
		public static VariableContainer<TIdent>.IntVariable Int<TIdent>(this VariableContainer<TIdent> container, TIdent ident)
		{
			if (!container.TryGet<VariableContainer<TIdent>.IntVariable>(ident, out var variable))
			{
				if (!container.AutoCreate)
					return null;

				variable = container.Create(new VariableContainer<TIdent>.IntVariable(ident));
			}
		...
		/// different type.
		/// </exception>
		public static VariableContainer<TIdent>.IntVariable Int<TIdent>(this VariableContainer<TIdent> container, TIdent ident, int value)
		{
			if (!container.TryGet<VariableContainer<TIdent>.IntVariable>(ident, out var variable))
				return container.Create(new VariableContainer<TIdent>.IntVariable(ident, value));
