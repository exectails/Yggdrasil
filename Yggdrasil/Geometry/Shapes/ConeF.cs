...
namespace Yggdrasil.Geometry.Shapes
{
	...
	public class ConeF : IShapeF
	{
		...
		/// </summary>
		public Vector2F Tip { get; private set; }
		...
		public ConeF(Vector2F tipPos, double direction, double radius, float angle)
		{
			this.Tip = tipPos;
		...
		private void UpdateCenter(Vector2F tipPos, double direction, double radius, float angle)
		{
			...
			this.Center = new Vector2F((float)Math.Round(centerX), (float)Math.Round(centerY));
		}
		...
		/// <returns></returns>
		public bool IsInside(Vector2F checkPos)
		...
		/// <returns></returns>
		public static bool IsInside(Vector2F tip, double radius, double direction, double angle, Vector2F checkPos)
		{
			if (angle <= 0)
				return false;
		...
		public Vector2F[] GetEdgePoints()
		{
			if (_edgePoints == null)
			{
				...
				_edgePoints = new Vector2F[]
				{
					this.Tip,
					new Vector2F((float)Math.Round(tx2), (float)Math.Round(ty2)),
					new Vector2F((float)Math.Round(tx5), (float)Math.Round(ty5)),
					new Vector2F((float)Math.Round(tx1), (float)Math.Round(ty1)),
				};
			}
