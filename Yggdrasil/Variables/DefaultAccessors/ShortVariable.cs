namespace Yggdrasil.Variables.DefaultGetters
{
	...
	public static partial class VariableContainerExtensions
	{
		...
		/// different type.
		/// </exception>
		public static VariableContainer<TIdent>.ShortVariable Short<TIdent>(this VariableContainer<TIdent> container, TIdent ident)
		{
			if (!container.TryGet<VariableContainer<TIdent>.ShortVariable>(ident, out var variable))
			{
				if (!container.AutoCreate)
					return null;

				variable = container.Create(new VariableContainer<TIdent>.ShortVariable(ident));
			}
		...
		/// different type.
		/// </exception>
		public static VariableContainer<TIdent>.ShortVariable Short<TIdent>(this VariableContainer<TIdent> container, TIdent ident, short value)
		{
			if (!container.TryGet<VariableContainer<TIdent>.ShortVariable>(ident, out var variable))
				return container.Create(new VariableContainer<TIdent>.ShortVariable(ident, value));
